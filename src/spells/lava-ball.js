...
export const LAVA_BALL = Object.freeze({
  name: "Lava Ball",
  key: 2,
  castTime: 1.5,
  damage: 240,
...
export class LavaBall extends FireBlast {
  ...
  release(args, origin) {
    ...
    const length =
      Math.hypot(end.x - origin.x, end.y - origin.y, end.z - origin.z) || 0.001;
    this.flight = {
      position: vec(origin),
      end,
      target,
  ...
  advance(dt, raycast) {
    ...
    const remaining = Math.hypot(
        ...
        f.end.y - f.position.y,
        f.end.z - f.position.z,
      ),
      step = Math.min(remaining, this.config.speed * Math.max(0, dt));
    const next = {
      x: f.position.x + f.direction.x * step,
      y: f.position.y + f.direction.y * step,
      z: f.position.z + f.direction.z * step,
    };
    const collision = raycast(f.position, next);
    // Missing collision data fails closed. Never tunnel through an unknown world.
    if (!collision) {
      this.flight = null;
      this.lastResult = "Collision world unavailable";
      return { ok: false, position: vec(f.position), reason: this.lastResult };
    }
    ...
    if (collision.hasHit || arrived || f.age > 4) {
      this.flight = null;
      const point =
        collision.hasHit && collision.hitPoint ? vec(collision.hitPoint) : next;
      const stillAtAim =
        Math.hypot(
          f.target.position.x - f.end.x,
