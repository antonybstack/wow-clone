...
const OUT=`struct Out{@builtin(position) position:vec4<f32>,@location(0) p:vec3<f32>,@location(1) uv:vec2<f32>,@location(2) color:vec4<f32>,@location(3) normal:vec3<f32>};`;
...
export async function surface(engine,name,url,{tint=[1,1,1],light=.6,alpha=false,wind=false,emission=0,pixels=128,uvScale=1,ground=false}={}){
 ...
 let lamp1=vec3<f32>(-3.4,2.5,12.0);let lamp2=vec3<f32>(3.6,2.7,14.0);
 let lamp=.45/(1.0+pow(distance(i.p,lamp1)*.5,2.0))+.4/(1.0+pow(distance(i.p,lamp2)*.5,2.0));
