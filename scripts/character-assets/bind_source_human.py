"""Human rest-mesh preparation for the original source deformation rig.
...
bpy.ops.import_scene.gltf(filepath=str(ROOT/'public/characters/bodies/human-animated-v1.glb'))
...
# Repose existing Human skin from A into the SOURCE rest-arm directions with
...
# Keep anatomical Human joint centers elsewhere; weights on helper/face bones
...
# joint frames/curves, assembled offline in bind-source-human.mjs.
...
bpy.ops.export_scene.gltf(filepath=str(OUT/'human-source-rest.glb'),use_selection=True,export_format='GLB',export_yup=True,export_animations=False,export_skins=True,export_all_influences=False,export_materials='EXPORT',export_cameras=False,export_lights=False)
def yup(p):return [p[0],p[2],-p[1]]
(OUT/'human-source-joints.json').write_text(json.dumps({n:yup(p) for n,p in points.items()},indent=2))
