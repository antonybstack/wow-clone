/** Assemble the source-compatible Undead candidate on the original 65-joint bind.
 *
 ...
 * the stored centimetre units, recompute the inverse binds, attach the Undead
 ...
 * base.glb lacks, so the Undead can be picked anywhere the Human can.
 *
 * Writes public/characters/candidates/undead-source-v1.glb + provenance.
 */
...
const REST = '.cache/source-motion/undead-source-rest.glb';
const JOINTS = '.cache/source-motion/undead-source-joints.json';
const OUT = 'public/characters/candidates/undead-source-v1.glb';
const PROVENANCE = 'public/characters/candidates/undead-source-v1.provenance.json';
...
// Drop the mannequin meshes, materials and images; the Undead brings its own.
...
// Attach the Undead surfaces with a palette remap into the final 65-joint order.
...
await fs.writeFile(PROVENANCE, JSON.stringify({
    pipeline: 'source-compatible Undead binding from Tripo Mixamo FBX',
    hashes: {
        ...
        [OUT]: sha(bytes),
        'scripts/character-assets/undead_from_tripo.py': sha(await fs.readFile('scripts/character-assets/undead_from_tripo.py')),
        'scripts/character-assets/bind-source-undead.mjs': sha(await fs.readFile('scripts/character-assets/bind-source-undead.mjs')),
        'blender/characters/sources/undead-tripo/provenance.json': sha(await fs.readFile('blender/characters/sources/undead-tripo/provenance.json')),
    },
    versions: {blender: '5.2.1', gltfTransform: versions['@gltf-transform/core'], glMatrix: versions['gl-matrix']},
    source: {
        title: 'Tripo Mixamo auto-rig from undead-body-reference-v1',
        job: 'https://studio.tripo3d.ai/workspace/rigging/bipedal-skeletal-humanoid-creature-with-detailed-bone-texture-and-musc-c2c5935d-97cb-49b1-9b23-2bc92dbc762d',
        files: 'blender/characters/sources/undead-tripo/undead-tripo.fbx',
    },
    licenseNotes: 'Body is a Tripo generation from the user-approved undead body study. Source rig and original clips retain base.glb provenance.',
}, null, 2) + '\n');
