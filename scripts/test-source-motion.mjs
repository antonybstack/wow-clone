...
const source=load('public/characters/base.glb'),candidate=load('public/characters/candidates/human-source-v1.glb');
...
test('source-compatible Human preserves every source rotation key, interpolation and timestamp',()=>{
 assert.equal(candidate.json.animations.length,source.json.animations.length);
...
test('new skin binds are inverses of the fitted source hierarchy',()=>{
 ...
 assert.deepEqual(j.meshes.map(m=>m.name).sort(),['HumanBody','HumanBrows','HumanEyes','HumanHair','HumanShorts'].sort());
...
test('diagnostic gameplay routes select original clips without procedural mage composition',()=>{
 for(const id of ['source-reference','human-source']){
  ...
  const clips=resolvePlayableClips(candidate.json.animations.map(a=>a.name),d);assert.equal(clips.clips.run,'Sprint_Loop');assert.equal(clips.clips.walk,'Walk_Loop');
 }
});
