...
const total=await page.evaluate(legs=>{
 ...
 const apply=p=>{
  A.player.setWorldPos(p.x,A.world.groundHeight(p.x,p.z)+1.7,p.z);
...
await fs.writeFile(`${dir}/recording.json`,JSON.stringify({tag,frames:frames.length,seconds:+span.toFixed(2),fps:+(frames.length/Math.max(span,.001)).toFixed(1),stats,post,errors,legs:LEGS},null,2));
