...
const errors=[],frames=[],writes=[],report={baseline,errors};let cdp;
...
try{
 ...
 for(let i=0;i<(baseline?15:185);i++){await p.waitForTimeout(200);const s=await p.evaluate(()=>({x:ASHEN.player.body.position.x,y:ASHEN.player.body.position.y,z:ASHEN.player.body.position.z,physics:ASHEN.player.getDebugState().usingPhysics,state:ASHEN.player.getDebugState()}));samples.push(s);if(!baseline&&views.length&&s.z>=views[0]){await p.keyboard.up('KeyW');await p.screenshot({path:`${dir}/route-${views.shift()}.png`});await p.keyboard.down('KeyW');}if(!baseline&&s.z>344)break;}
 await p.keyboard.up('KeyW');report.samples=samples;
 await p.screenshot({path:`${dir}/destination.png`});
 if(baseline)assert(samples.at(-1).z<=143.01,'Reproduce old clamp');
 else{assert(samples.at(-1).z>344,'Reach cathedral nave on foot');assert(samples.every(s=>Number.isFinite(s.y)&&s.physics));assert.equal(samples.at(-1).state.recoveries,samples[0].state.recoveries,'No fall recovery disguising route failure');assert(samples.some(s=>s.z>145));await p.evaluate(()=>{ASHEN.rig.yaw=Math.PI;ASHEN.rig.pitch=-.28;ASHEN.rig.distance=ASHEN.rig.distanceTarget=4;});await p.waitForTimeout(700);await p.screenshot({path:`${dir}/nave-return.png`});await p.keyboard.down('KeyS');for(let i=0;i<100;i++){await p.waitForTimeout(200);if(await p.evaluate(()=>ASHEN.player.body.position.z)<298)break;}await p.keyboard.up('KeyS');report.returnZ=await p.evaluate(()=>ASHEN.player.body.position.z);assert(report.returnZ<298,'Exit the nave back into courtyard');await p.screenshot({path:`${dir}/courtyard-return.png`});}
 if(record){await cdp.send('Page.stopScreencast');await Promise.all(writes);const lines=['ffconcat version 1.0'];frames.forEach((f,i)=>lines.push(`file 'frames/${f.name}'`,`duration ${Math.max(.008,(frames[i+1]?.ts??f.ts+1/60)-f.ts)}`));lines.push(`file 'frames/${frames.at(-1).name}'`);await fs.writeFile(`${dir}/frames.ffconcat`,lines.join('\n')+'\n');report.recording={frames:frames.length,seconds:frames.at(-1).ts-frames[0].ts};}
...
}catch(e){report.failure=e.stack;throw e;}finally{await p.keyboard.up('KeyW').catch(()=>{});await fs.writeFile(`${dir}/report.json`,JSON.stringify(report,null,2));console.log(JSON.stringify({...report,samples:report.samples?.length}));await c.close();await b.close();}
