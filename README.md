...
The displacement is real geometry in the beauty pass _and_ in all three shadow
...
crest and ballistic grains flung clear — emitted at _fractional_ positions along
...
4. **Crystallise** — hexagonal prisms grown along a golden-angle spiral,
   alpha-blended _and_ depth-writing, so you see the snow through the ice but
...
5. **Vortex** — three helices of lifted snow winding around the player, with the
   ...
   velocity. The only system here that writes a _negative_ depression.
...
_through_ a berm crest rather than putting a bright patch on the near face of
...
- **TAA** — Halton(2,3) jitter written straight into the projection and frozen
  ...
  history fetch.
- **Bloom** — three levels, thresholded in _exposed_ units so only the sun disc,
