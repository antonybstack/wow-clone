...
Choose one consequential deliverable from the latest user request. The next tracked deliverable is the first distinct Orc fit in [the armory plan](armory-and-equipment-plan.md) (Stage D), after the two-handed prop/pose and stow/draw proof delivered below. Geometric seam qualification beyond current fits stays open. Preserve approved work, but replace design/code that contradicts a new explicit target. Do not pursue old milestone lists, build generalized infrastructure ahead of a convincing playable slice, or confuse passing tests with visual acceptance.
