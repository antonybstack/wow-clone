...
- Bind: Mixamo T-pose, 65 joints, 55 clips, pelvis-only loincloth weights. Pack: `public/ashen-reach/equipment-orc/` with the same eight catalogue garments as Human. Orc rest meshes are limb-graded (boots enclose the print foot/calf; gloves are a short gauntlet). `OrcV1Body` is split into the Human coverage geosets so boots/gloves hide skin.
- Remaining polish: idle thoracic stoop is Mixamo idle on the print chest (T-pose rest is upright enough; no clip-fighting spine hack), chin pad still a slight pale catch. Boot last UVs stretch where a Human Viking boot was graded onto a print foot. Do **not** add a runtime wrap engine. Factory props already have an Orc grip correction (×1.16 and a short palm nudge); Mixamo two-hand carry still reads as a pole across the chest. Loincloth inner crotch loops filled 2026-09-19. Face: print eyelids/lips/tusks, HP normals, no dummy spheres, pec ivory cleared. Palette ashen peat-olive.
