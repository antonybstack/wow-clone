...
- Reviewed video: [two-handed walkthrough](https://ve.sparkify.dev/wow-clone/ashen-reach/two-handed/2026-09-18-two-handed-walkthrough.mp4) — 14.7 s, 1280×720 H.264/AAC with real engine audio, recorded by `scripts/ashen-reach/record-two-handed.mjs` (CDP screencast + `audio.capture()`). Covers armory Warden equip, carry pose front/side, live carry walk, Fire Blast stow/release/recovery, Lava Ball stationary charge/release/recovery and re-grip. Verified HTTP 200, `video/mp4`, matching 14,849,571 bytes and HTTP 206 ranges; delivered to the authorized Telegram chat (message 564). Stills: `ve-capture/ashen-reach/two-handed-v7/`. `ffmpeg` is not on PATH — the bundled `/Applications/BabylonJS Editor.app/Contents/bin/ffmpeg` (7.1, libx264/aac) encoded the `frames.ffconcat` + `audio.webm` with the recorded `audioOffset`.
