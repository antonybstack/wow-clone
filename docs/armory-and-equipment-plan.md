...
- Grok worker evidence: `ve-capture/orc-motion/grok-v5/` (8 stills, head crops, `orc-head-montage.png`, `skin-detail.png`, `RESULT.md`); intermediate passes in `grok-v1..v4/`. Independent parent review of the final montage: **convinces as a stylized orc for this art direction** — matte credible skin, tusks read as a defining feature in three-quarter and profile, brow/muzzle/jaw in place. **Remaining polish, not blockers:** the pale band across the muzzle competes with the tusks in pure front view, skin tone variation is still flat versus the reference, and the geometric topknot reads hard against the softer body. It is not a Grommash sculpt match. All gates green after every pass: 5/5 motion tests, validator 0 errors, lean Δ0.00°, build.
- Armory race switch (2026-09-18): the Race selector is now functional. Selecting **Orc** swaps the live armory character through a new `body.swapSource(url)` / `body.restoreSource()` pair that assembles a second source-compatible visual and **parks** the Human one without retiring it, so equipment keeps its palette, its bound garment meshes and the remembered outfit, and the return is exact. Equipment slots are disabled with an explicit "Orc equipment fits are not ready — base appearance only" status while the Human selection is untouched; camera framing scales ×1.22 for the 2.14 m body. `bind-source-orc.mjs` now also copies the **ten runtime-only clips** the game selects (5 directional gaits, 4 split cast layers, the retargeted two-handed carry) from the authoring pack, so the Orc exposes every pose preview including Fire Blast, Lava Ball and carry. Gameplay persistence is deliberately **not** in this slice: the armory restores Human on close because Orc garment/socket fits do not exist yet. Validation: **26 armory checks** (4 new race-switch checks), 23 equipment tests, the two-handed/graveweaver/grip/stream browser checks and the build all pass; evidence `ve-capture/ashen-reach/armory-orc/` and independent image review found the Orc intact in all poses with no Human clothing on it.
- Orc garment fits (2026-09-18, delegated Grok 4.6 worker — 3 resumed passes): `scripts/ashen-reach/fit-orc-clothes.py` adapts the Human MakeClothes pipeline. The MakeHuman→GLB vertex index correspondence does **not** hold (a `k`-index mapping is off by a median 0.39 m), so the bulk is transferred by KD-tree nearest neighbour from `orc-animated-v1.glb` to the bulked mesh (`.cache/source-motion/orc-bulked.glb`) for the body prefix, helpers undeltad; garments are then fitted in that bulked cloud, bisected for the 2.10 m stature, reposed to the **Orc** source-rest directions, and remapped to `mixamorig:*` with the 65-joint armature rebuilt from `orc-source-v1.glb`'s joint positions. Output `.cache/armory-assets/orc-apparel-rest.glb` (8 garments + pendant) plus `blender/characters/ashen-orc-wayfarer.blend`. `prepare-orc-equipment-preview.mjs` merges an outfit with the Orc and `review-orc-fits.mjs` captures per-outfit stills. Three fixes found by review: the hood had been picking up **shoulder bulk displacement** (now fades to zero at the neck, so it sits on the skull with the face in the opening); the pale chest blotch was the tunic's **silver beast emblem**, crushed by packing the albedo at 256 px nearest-neighbour (now 512 px linear and reads as embroidery); the crotch/fly gap was closed by dropping the tunic hem and raising the trousers. **Honest limits:** a fly sliver can still appear mid-walk, the robe skirt is slabby in sprint, and the tusks read long in pure side view under the hood. Evidence `ve-capture/orc-motion/orc-fits-v3/` (18 stills, 3 outfits); gates stayed green throughout (5/5 motion tests, validator 0 errors, lean Δ0.00°). Runtime equipping of Orc garments — a race-aware equipment pack/manifest and socket refits — is the next slice.
...
```sh
/Applications/Blender.app/Contents/MacOS/Blender --background --python scripts/character-assets/bulk_orc.py
