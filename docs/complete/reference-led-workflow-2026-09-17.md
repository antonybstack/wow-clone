...
## Workflow adjustment — 2026-09-18 (Orc / delegation lessons)

The Orc work (bind → Grommash-scale anatomy → armory race switch → garment fits) ran mostly as **resumed Grok 4.6 CLI passes** with parent review of every pass. The loop stayed the same; these adjustments were earned and are now expected:

1. **Delegate bounded, script-heavy asset passes; keep the parent on judgment and integration.** Eight passes produced the bulked Orc, the face/tusk/material pass, the garment fittings and the fixes. Each handoff froze the allowed paths, the invariants (65 joints, 45/55 clips, bit-identical rotations, untouched Human assets), the exact verification commands and the evidence directory; each pass had one ordered defect list with a physical hypothesis.
2. **Isolate the acceptance view.** Require per-outfit (per-variant) previews. The all-items composite created false cloth-through-cloth defects; the single-outfit stills located the real hood bug (shoulder displacement leaking into the cowl).
