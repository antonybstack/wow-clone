...
All seven slots, contrasting cloth/mail/magic outfits and staff/sword/book choices are implemented. The `graveweaverGreatstaff` is a real two-handed variant (see below); a shield does not exist yet. The same logical items now have a print-sculpt Orc fit (`scripts/ashen-reach/fit-orc-garments.mjs` + `prepare-orc-equipment.mjs`); MHCLO cannot map that topology. Undead and arbitrary body sliders remain unproven.

## Two-handed hold and eased stow/draw (2026-09-18)

- `graveweaverGreatstaff` uses `factory:'greatstaff'` (a longer shaft/fork variant of `createMageProp('staff')`), `twoHanded:true`, and `occupies:['mainHand','offHand']`. Its `gripPosition`/`gripRotation` are authored in the **evaluated right-hand socket frame** so the shaft passes through the right wrist and the left hand. Current values (verified at the neutral hold phase below): `gripPosition:[.00572,.02636,-.01291]`, `gripRotation:[-.353791,0,-.362037,.862416]`. Re-derive them whenever the carry pose or weapon changes.
- The carry pose is a **retargeted CC0 animation**, not a hand-authored override. `node scripts/ashen-reach/append-carry.mjs` imports Quaternius UAL2 `Walk_Carry_Loop` (CC0; `.cache/animation-research/ual2/UAL2_Standard.glb`) onto the source rig with the vendored MIT retargeter and records provenance in `animation-provenance.json`. `body.js` (`updateCarry`) layers it on the arms only — shoulders through hands — over the normal directional gait while the greatstaff is equipped, keeping its moving cadence (`WALK_RATIO`/`RUN_RATIO`). The carry clip and the locomotion/jump clips wear complementary masks (`carryMask`/`carryLocoMask` in `body-visual.js`), so a joint is never driven by both. Until 2026-09-18 it was the whole-body locomotion pose, which replaced directional leg motion; do not restore that. Stationary, the arm cycle eases to its most neutral phase (`TWO_HAND_STILL_TIME = 0.9` s into the 2.0 s loop, exported from `body-visual.js`) within 0.25 s instead of parking whatever lean-back walk extreme the playhead stopped at — a staff is not a heavy weapon, so the idle hold stays close to a normal stance. The phase comes from `node scripts/ashen-reach/scan-carry-neutral.mjs`, which ranks loop phases by torso/hip/arm/leg deviation from bind; the 0.87–0.93 s plateau is flat, the worst phase is ~1.38 s. Contact at the hold phase measures right ~0 cm, left ~1.1 cm off the shaft axis (check budget ≤2 cm), so the grip needed no change. Paired-phase image review confirms the 0.9 s hold reads upright and relaxed with both hands on the shaft while the 1.38 s phase reads braced and leaned-back. `scripts/ashen-reach/sync-equipment-clips.mjs` copies a clip into `wanderer-equipment.glb` when only animation changed (avoids the Blender/garment recomposition), then `npm run prepare:equipment` re-splits the streamed assets. The earlier numeric solver (`solve-two-hand-pose.mjs`), Blender IK exporter and `two-hand-carry-pose.json` are **superseded** — do not restore them or a `Pistol_Idle_Loop`/`Push_Loop` overlay (a source arm pose re-pitches when applied over locomotion).
...
- The armory's Graveweaver preset button equips all seven slots. Full-body framing stays the same size as the other presets, so the body does not jump when the staff is equipped. Individual controls remain independent; closing keeps the chosen combination.
