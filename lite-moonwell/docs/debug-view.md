...
From `lite-moonwell`, run `npm run dev`, then open `http://127.0.0.1:5180/ashen-reach.html?play&clean`. Current game global: **ASHEN**, readiness: `ASHEN.ready`. The character lab and body preview expose diagnostic globals only; do not use their state or frame rates as Ashen evidence.
...
await page.goto('http://127.0.0.1:5180/ashen-reach.html?play&clean', {waitUntil:'commit'});
...
The VE host is **`https://ve.sparkify.dev`**, backed by Cloudflare R2 bucket `fardel-ve`. A Telegram attachment does not automatically create a URL there. When public sharing is authorized, upload the reviewed video with the existing helper from `lite-moonwell`:
