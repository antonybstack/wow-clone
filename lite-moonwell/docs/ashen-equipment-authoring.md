...
Run from `lite-moonwell`. Existing base MakeHuman sources and original source-compatible `wanderer.glb` must already exist; the garment fetch does not recreate the actor.
...
Vite 5180, owned Chrome CDP 9337. Run browser scripts sequentially against `ashen-reach.html?play&clean`:
