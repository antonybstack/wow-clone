...
The only playable game route is **Ashen Reach**, `ashen-reach.html`, in `lite-moonwell` (Vite **5180**). Open `http://127.0.0.1:5180/ashen-reach.html?play&clean`. `character-lab.html` and `body-preview.html` are retained development tools; they are not game routes or the current scene/art target.
...
| --- | --- |
| Engine | `@babylonjs/lite`, WebGPU; Vite 5180. No `@babylonjs/core`/Classic runtime APIs. |
