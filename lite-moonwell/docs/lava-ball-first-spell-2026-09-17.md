...
Open `http://127.0.0.1:5180/ashen-reach.html?play&clean`. Tab targets the training dummy. **1** remains Fire Blast; **2**, or its new clickable slot, casts Lava Ball. Press once rather than holding the key.
...
Browser harnesses use owned Chrome CDP **9337** and Vite **5180**. Capture output is ignored local working evidence. Recording reuses the existing CDP full-tab recorder and actual Lite audio mix; do not substitute staged sound.
